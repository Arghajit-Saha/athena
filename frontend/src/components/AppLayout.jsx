...
export default function AppLayout() {
  ...
  return (
    <div className="min-h-dvh bg-surface flex flex-col font-sans text-body relative selection:bg-accent selection:text-inverse">
        ...
        <nav className="pointer-events-auto w-full max-w-[720px] bg-white/60 dark:bg-card/60 backdrop-blur-2xl border border-lavender/30 dark:border-lavender/10 shadow-2xl shadow-accent/5 rounded-full p-2 flex items-center justify-between transition-all hover:border-lavender/50">
          ...
          <div className="flex-1 flex justify-start pl-1">
            <Link to="/" className="flex items-center gap-2.5 rounded-full no-underline" title="Home">
              ...
              <span className="font-sans font-bold tracking-tight text-[17px] text-heading pr-2 max-sm:hidden">Athena</span>
            </Link>
          </div>

          <div className="flex items-center justify-center gap-2 sm:gap-4 px-3 sm:px-10 border-x border-edge/40">
            {navItems.map(item => {
              ...
              return (
                <Link
                  key={item.href}
                  to={item.href}
                  className={`relative px-4 sm:px-6 py-2 sm:py-2.5 rounded-full text-[13px] sm:text-[13.5px] font-semibold transition-all duration-300 no-underline ${isActive
          ...
          <div className="flex-1 flex items-center justify-end gap-1 sm:gap-2 pr-1">
      ...
      <div className="h-[120px] max-md:h-[72px]" />
